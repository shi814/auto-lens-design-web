#!/usr/bin/env python3
"""Streamlit page for filtering existing lens results and exporting visuals."""
...
# ---------------------------------------------------------------------------
# 2) Run the second-stage (AirGap unsupervised) test on the generated set.
# ---------------------------------------------------------------------------
def _build_test_opt(out_dir: Path, batch_size: int):
    """Build a Test_Model opt namespace by reusing utils.set_parser defaults.

    sys.argv is temporarily overridden because set_parser() reads argparse from
    it; the Streamlit process argv would otherwise be parsed and rejected.
    """
...
def run_second_stage_test(test_csv: Path, out_dir: Path, *, batch_size: int = 128) -> Path:
    """Run the AirGap second-stage test on the generated CSV, in-process.

    The test is executed inside the current process (no subprocess) so that
    only one Python interpreter ever holds PyTorch in memory. This keeps the
    peak memory low enough for CPU-only free hosting and avoids re-importing
    torch on every request. Lightweight mode is used so only the metrics CSV
    is produced; filtering, glass matching, export and visuals are handled by
    the web pipeline. Returns the path to the metrics CSV.
    """
...
def _display_one_system(data: dict) -> None:
    ...
    # Row 2: a larger spot diagram and a smaller, vertically-centered distortion plot.
    c1, c2 = st.columns([7, 3], gap="large", vertical_alignment="center")
    with c1:
