...
class USL_Loss(nn.Module):
    ...
    def raytrace_all(self, lens: LensBatch, max_surf_lens, surf_lens, mask):
        ...
        # Stable trace-EFL path. The legacy acos/tan expression above has a
        # singular gradient when cos(theta) reaches +/-1. Using the direction
        # ratio is algebraically equivalent for finite rays and avoids that
        # backward singularity.
        slope_floor = max(float(getattr(self.opt, "efl_slope_floor", 1e-6)), float(eps))
        transverse = torch.sqrt(
            torch.clamp(VX_edge ** 2 + VY_edge ** 2, min=slope_floor ** 2)
        )
        EFL_est_g = (self.EPD / 2.0) * VZ_edge / transverse
        efl_cap_factor = max(float(getattr(self.opt, "efl_cap_factor", 10.0)), 1.0)
        efl_cap = torch.abs(EFL_ideal).clamp_min(float(self.EPD)) * efl_cap_factor
        EFL_est_g = torch.nan_to_num(EFL_est_g, nan=0.0, posinf=1e6, neginf=-1e6)
        EFL_est_g = torch.maximum(torch.minimum(EFL_est_g, efl_cap), -efl_cap)
